...
begin
  ...
  select stock, price into v_stock, v_price from product where product_id = p_product_id for update;
  if not found then
    return jsonb_build_object('code','not_found','message','Product not found');
  end if;

  select quantity into v_exists_qty from invoiceproduct where invoice_id = p_invoice_id and product_id = p_product_id;
  ...
  if v_exists_qty is null then
    ...
    update invoiceproduct set quantity = quantity + p_quantity
  ...
  -- Recalculate invoice total: products + service bookings
  ...
  return jsonb_build_object('code','success','message','Product added to invoice');
